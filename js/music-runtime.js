...
(function(){
  ...
  function status(t){for(const id of ['musicProviderStatus','musicPlayerStatus','musicScreenStatus']){const e=$(id);if(e)e.textContent=t}}
  ...
  async function loadFavorites(){
    try{
      ...
      if(r.error)throw r.error;if(ctx().user?.id!==user.id)return;
  ...
  async function loadEventPlaylist(){
    try{
      ...
      if(ctx().event?.id!==event.id)return;renderPlaylist(playlist,items);
  ...
  function renderPlaylist(playlist,items){
    ...
    panel.classList.add('open');window.dispatchEvent(new CustomEvent('music-event-loaded',{detail:{playlist,items}}));
  ...
  function openPlayer(track,items=[]){
    ...
    panel.classList.add('open');panel.scrollTop=0;window.dispatchEvent(new CustomEvent('music-player-open'));
  ...
  async function addTrackToEvent(track,button){
    ...
    try{
      ...
      status('Morceau ajouté à la playlist 🎉');if(button){if(button.querySelector('span'))button.querySelector('span').textContent='Ajouté';else button.textContent='✓'}await loadEventPlaylist();return true;
  ...
  nav.addEventListener('click',e=>{const b=e.target.closest('[data-bottom-tab="music"]');if(b){e.preventDefault();e.stopImmediatePropagation();if(window.MyEventMusicWorkspace)window.MyEventMusicWorkspace.go(window.MyEventMusicSession.get().screen,false);else openMusic()}},true);
  nav.addEventListener('click',e=>{if(e.target.closest('[data-bottom-tab]')?.dataset.bottomTab!=='music')closeMusic();},true);
