...
(function(){
  ...
  let searchVersion=0;
  async function search(q){if(!q?.trim()||q.trim().length<2)return;const version=++searchVersion;const view=revision;notice('Recherche YouTube…');try{const r=await fetch('/api/search-music?q='+encodeURIComponent(q));const data=await r.json();if(!r.ok)throw Error(data.error||'Recherche indisponible');if(version!==searchVersion||view!==revision)return;results=data.items||[];S.update({query:q});const box=$('musicDiscoverResults');if(box){box.replaceChildren();rows(results,box);}notice(results.length+' résultat(s)');}catch(e){if(version===searchVersion)notice(e.message);}}
