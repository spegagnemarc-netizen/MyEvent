...
(function(){
  ...
  function editable(key,body){
    const list=S.get()[key];rows(list,body,(row,t,i)=>{
      ...
      const lockDraft=button(t.locked?'🔒':'🔓',e=>{e.preventDefault();e.stopPropagation();const item=S.get()[key]?.[i];if(!item)return;item.locked=!item.locked;S.save();go(screen,false);},row);lockDraft.classList.add('musicDraftLock');lockDraft.title=t.locked?'Déverrouiller le morceau':'Verrouiller le morceau';lockDraft.setAttribute('aria-label',lockDraft.title);lockDraft.setAttribute('aria-pressed',String(!!t.locked));
  ...
  function renderHome(){
    ...
    extra.innerHTML='<div class="musicCards musicHeroCards"><button data-go="playlists" class="musicIllustrated musicHeroPlaylists" aria-label="Mes playlists"><img class="musicPlaylistArt" src="assets/music-playlists-provided.png" alt=""><span class="musicHeroIcon">♫</span><b>Mes playlists</b><small>Vos playlists personnelles</small><i>›</i></button><button data-go="discover" class="musicIllustrated musicHeroDiscover"><span class="musicHeroIcon">▶</span><b>Découvrir</b><small>Explorer YouTube</small><i>›</i></button><button data-go="ai" class="musicIllustrated musicHeroAI"><span class="musicHeroIcon">✦</span><b>Music IA</b><small>Composer une proposition</small><i>›</i></button><button data-go="library" class="musicIllustrated musicHeroLibrary"><span class="musicHeroIcon">♥</span><b>Bibliothèque</b><small>Favoris et historique</small><i>›</i></button></div><div class="musicForYouHead"><h3>Pour vous</h3><button type="button" data-go="discover">Voir tout ›</button></div><p>Retrouvez vos écoutes et explorez vos ambiances favorites.</p><section class="musicShelf"><div class="musicShelfHead"><h3>◷ Écoutés récemment</h3><button type="button" data-go="library">Voir tout ›</button></div><div id="musicRecent"></div></section><section class="musicShelf musicGenreShelf"><div class="musicShelfHead"><h3>♫ Genres</h3><button type="button" data-music-filter="genres">Voir tout ›</button></div><div id="musicGenres"></div></section>';
    ...
    // Final Music home order: recent -> genres -> ambiances -> trends -> MyEvent playlists.
    const moodSection=home.querySelector('#musicAmbiancesSection');
    const trendSection=home.querySelector('#musicTrending')?.closest('.musicSection');
    const playlistSection=home.querySelector('#musicPlaylistsSection');
    if(moodSection)home.appendChild(moodSection);
    if(trendSection)home.appendChild(trendSection);
    if(playlistSection)home.appendChild(playlistSection);
    loadHomeTrends();
  }
