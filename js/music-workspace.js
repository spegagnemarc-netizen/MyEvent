...
(function(){
  ...
  let screen='home',history=[],results=[],eventData=null,revision=0;
  ...
  function discover(body){body.innerHTML='<form id="musicDiscoverForm"><label>Rechercher sur YouTube<input id="musicDiscoverQuery" type="search" minlength="2" required></label><button>Rechercher</button></form><div id="musicDiscoverResults"></div>';$('musicDiscoverQuery').value=S.get().query;rows(results,$('musicDiscoverResults'));
    $('musicDiscoverForm').onsubmit=e=>{e.preventDefault();search($('musicDiscoverQuery').value);};
  }
  let searchVersion=0;
  let searchController=null;
  async function search(q,artContext){q=q?.trim();if(!q||q.length<2)return;if(artContext)S.update({artContext});const version=++searchVersion;const view=revision;searchController?.abort();searchController=new AbortController();const controller=searchController;S.update({query:q});const input=$('musicDiscoverQuery');if(input)input.value=q;const box=$('musicDiscoverResults');if(box)box.replaceChildren();notice('Recherche « '+q+' »…');try{const r=await fetch('/api/search-music?q='+encodeURIComponent(q),{signal:controller.signal,cache:'no-store'});const data=await r.json();if(!r.ok)throw Error(data.error||'Recherche indisponible');if(controller!==searchController||version!==searchVersion||view!==revision)return;results=data.items||[];const current=$('musicDiscoverResults');if(current){current.replaceChildren();rows(results,current);}notice(results.length+' résultat(s) · '+q);}catch(e){if(e.name==='AbortError')return;if(controller===searchController&&version===searchVersion){results=[];const current=$('musicDiscoverResults');if(current)current.replaceChildren();notice(e.message);}}}
  ...
  function renderHome(){
    ...
    extra.innerHTML='<div class="musicCards"><button data-go="discover" class="musicIllustrated">⌕<b>Découvrir</b><small>Explorer YouTube</small></button><button data-go="ai" class="musicIllustrated">✦<b>Music IA</b><small>Composer une proposition</small></button><button data-go="library" class="musicIllustrated">♥<b>Bibliothèque</b><small>Retrouver vos titres</small></button></div><h3>Pour vous</h3><p>Retrouvez vos écoutes et explorez vos ambiances favorites.</p><h3>Écoutés récemment</h3><div id="musicRecent"></div><h3>Genres</h3><div id="musicGenres"></div>';
    extra.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>go(b.dataset.go));rows(S.get().recent.slice(0,3),$('musicRecent'));
    for(const genre of ['Pop','Jazz','Rock','Électro','Classique']){const key='genre:'+genre.toLowerCase().normalize('NFD').replace(/[\\u0300-\\u036f]/g,'');const b=button(genre,()=>{go('discover');search(genre,key);},$('musicGenres'));b.classList.add('musicGenreCard');b.dataset.artKey=key;const art=S.get().artwork[key]?.thumbnail_url;if(art)b.style.backgroundImage='linear-gradient(180deg,rgba(5,8,12,.06),rgba(5,8,12,.78)),url("'+art.replace(/"/g,'%22')+'")';}
  }
