...
(function(){
  ...
  function renderHome(){
    ...
    const genres=['Pop','Rap','Hip-Hop','R&B','Jazz','Rock','Électro','Dance','Variété française','Latino','Reggae','Classique','K-Pop','Métal','Afro','Country','Soul','Funk','Années 80','Années 90','Années 2000'];for(const genre of genres){const key='genre:'+genre.toLowerCase().normalize('NFD').replace(/[\\u0300-\\u036f]/g,'');const b=button(genre,()=>{go('discover');search(genre,key);},$('musicGenres'));b.classList.add('musicGenreCard');b.dataset.artKey=key;const art=S.get().artwork[key]?.thumbnail_url;if(art)b.style.setProperty('background-image','linear-gradient(180deg,rgba(5,8,12,.06),rgba(5,8,12,.78)),url("'+art.replace(/"/g,'%22')+'")','important');}
  }
